...
#$((1 + RANDOM % 2147483647))
javaCommand="java -Xmx30G -jar algorithms/binaries/elki-kmeans--star.jar KDDCLIApplication"
...
process_synhtetic_dataset() {
	...
	echo "k,l,auroc,auprc,auprgc,avgprec,rprec,f1,dcg,ndcg,adj-auroc,adj-auprc,adj-auprgc,adj-avgprec,adj-rprec,adj-f1,adj-dcg,runtime" >> "$csvFile"

	for k in {1..50}; do
		local list=(25 50 75 100 150 200 250 300)

		for l in "${list[@]}"; do
			l=`echo "scale=4; $l / 10000" | bc -l`
			...
			for i in {1..5}; do
				for _ in {1..10}; do
					$javaCommand -algorithm $algorithm \
						...
						-kmeansmm.l $l \
						-kmeansmm.noisecluster true > $algorithm-$dataset\_$i.out
					awk '/kmeans.k|kmeansmm.l|runtime: |measures AUROC|Adjusted AUROC|measures AUPRC|Adjusted AUPRC|measures AUPRGC|Adjusted AUPRGC|measures Average Precision|Adjusted AveP|measures R-Precision|Adjusted R-Prec|measures Maximum F1|Adjusted Max F1|measures DCG|Adjusted DCG|measures NDCG /{
						...
						print kmeans_k","kmeansmm_l","auroc","auprc","auprgc","avg_prec","rprec","max_f1","dcg","ndcg","adj_auroc","adj_auprc","adj_auprgc","adj_avg_prec","adj_rprec","adj_max_f1","adj_dcg","runtime;
...
case "$1" in
    ...
    -s) _semantic ;;
	-syn) _synthetic ;;
    *)  echo "Usage: ./kmeans--star.sh [-l | -s | -syn]" ;;
esac