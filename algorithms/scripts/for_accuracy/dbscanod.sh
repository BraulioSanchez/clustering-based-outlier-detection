...
#$((1 + RANDOM % 2147483647))
javaCommand="java -Xmx30G -jar algorithms/binaries/elki-dbscanoutlier.jar KDDCLIApplication"
...
process_synhtetic_dataset() {
	...
	echo "epsilon,minpts,core,auroc,auprc,auprgc,avgprec,rprec,f1,dcg,ndcg,adj-auroc,adj-auprc,adj-auprgc,adj-avgprec,adj-rprec,adj-f1,adj-dcg,runtime" >> "$csvFile"
		...
		for j in "${list[@]}"; do
			...
			for i in {1..5}; do
				...
				awk '/dbscan.epsilon|dbscan.minpts|gdbscan.core-model|runtime: |measures AUROC|Adjusted AUROC|measures AUPRC|Adjusted AUPRC|measures AUPRGC|Adjusted AUPRGC|measures Average Precision|Adjusted AveP|measures R-Precision|Adjusted R-Prec|measures Maximum F1|Adjusted Max F1|measures DCG|Adjusted DCG|measures NDCG /{
					...
					print dbscan_epsilon","dbscan_minpts","gdbscan_coremodel","auroc","auprc","auprgc","rprec","max_f1","dcg","ndcg","adj_auroc","adj_auprc","adj_auprgc","avg_prec","adj_avg_prec","adj_rprec","adj_max_f1","adj_dcg","runtime;
...
case "$1" in
    ...
    -s) _semantic ;;
	-syn) _synthetic ;;
    *)  echo "Usage: ./dbscanod.sh [-l | -s | -syn]" ;;
esac