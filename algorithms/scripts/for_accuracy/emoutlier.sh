...
#$((1 + RANDOM % 2147483647))
javaCommand="java -Xmx30G -jar algorithms/binaries/elki-emoutlier.jar KDDCLIApplication"
		...
		for i in {1..5}; do
			for _ in {1..10}; do
				$javaCommand -algorithm $algorithm \
					...
					-em.map.prior 0.1 \
					-em.delta 1.0E-4 \
					-em.miniter 1 > $algorithm-$dataset\_$i.out
				awk '/em.k|runtime: |measures AUROC|Adjusted AUROC|measures AUPRC|Adjusted AUPRC|measures AUPRGC|Adjusted AUPRGC|measures Average Precision|Adjusted AveP|measures R-Precision|Adjusted R-Prec|measures Maximum F1|Adjusted Max F1|measures DCG|Adjusted DCG|measures NDCG /{
					...
					print em_k","auroc","auprc","auprgc","avg_prec","rprec","max_f1","dcg","ndcg","adj_auroc","adj_auprc","adj_auprgc","adj_avg_prec","adj_rprec","adj_max_f1","adj_dcg","runtime;
...
case "$1" in
    ...
    -s) _semantic ;;
	-syn) _synthetic ;;
    *)  echo "Usage: ./emoutlier.sh [-l | -s | -syn]" ;;
esac