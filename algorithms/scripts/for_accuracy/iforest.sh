...
process_synhtetic_dataset() {
	...
	echo "numtrees,subsample,auroc,auprc,auprgc,avgprec,rprec,f1,dcg,ndcg,adj-auroc,adj-auprc,adj-auprgc,adj-avgprec,adj-rprec,adj-f1,adj-dcg,runtime" >> "$csvFile"
		...
		for i in {1..5}; do
			for _ in {1..10}; do
				$javaCommand -algorithm $algorithm \
					...
					-iforest.subsample $psi > $algorithm-$dataset\_$i.out
				awk '/iforest.numtrees|iforest.subsample|runtime: |measures AUROC|Adjusted AUROC|measures AUPRC|Adjusted AUPRC|measures AUPRGC|Adjusted AUPRGC|measures Average Precision|Adjusted AveP|measures R-Precision|Adjusted R-Prec|measures Maximum F1|Adjusted Max F1|measures DCG|Adjusted DCG|measures NDCG /{
					...
					print iforest_numtrees","iforest_subsample","auroc","auprc","auprgc","avg_prec","rprec","max_f1","dcg","ndcg","adj_auroc","adj_auprc","adj_auprgc","adj_avg_prec","adj_rprec","adj_max_f1","adj_dcg","runtime;
