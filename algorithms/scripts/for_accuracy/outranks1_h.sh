...
#$((1 + RANDOM % 2147483647))
javaCommand="java -Xmx10G -jar algorithms/binaries/elki-outranks1_h.jar KDDCLIApplication"
algorithm="outlier.subspace.OutRankS1HDBSCAN"
alias="outlier.subspace.OutRankS1_H"
...
process_synhtetic_dataset() {
	...
	echo "minpts,minclsize,alpha,subspaces,auroc,auprc,auprgc,avgprec,rprec,f1,dcg,ndcg,adj-auroc,adj-auprc,adj-auprgc,adj-avgprec,adj-rprec,adj-f1,adj-dcg,runtime" >> "$csvFile"
	...
	for j in "${list[@]}"; do
		...
		for i in {1..5}; do
			...
			awk '/hdbscan.minPts|hdbscan.minclsize|outrank.s1.alpha|outrank.s1.subspaces|runtime: |measures AUROC|Adjusted AUROC|measures AUPRC|Adjusted AUPRC|measures AUPRGC|Adjusted AUPRGC|measures Average Precision|Adjusted AveP|measures R-Precision|Adjusted R-Prec|measures Maximum F1|Adjusted Max F1|measures DCG|Adjusted DCG|measures NDCG /{
				...
				print hdbscan_minPts","hdbscan_minclsize","outrank_s1_alpha","outrank_s1_subspaces","auroc","auprc","auprgc","avg_prec","rprec","max_f1","dcg","ndcg","adj_auroc","adj_auprc","adj_auprgc","adj_avg_prec","adj_rprec","adj_max_f1","adj_dcg","runtime;
...
case "$1" in
    ...
    -s) _semantic ;;
	-syn) _synthetic ;;
    *)  echo "Usage: ./outranks1_h.sh [-l | -s | -syn]" ;;
esac