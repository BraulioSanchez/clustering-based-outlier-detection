...
process_synhtetic_dataset() {
	...
	echo "k,scoring,auroc,auprc,auprgc,avgprec,rprec,f1,dcg,ndcg,adj-auroc,adj-auprc,adj-auprgc,adj-avgprec,adj-rprec,adj-f1,adj-dcg,runtime" >> "$csvFile"

	local scorings=("DISTANCE" "DISTANCE_SINGLETONS" "VARIANCE")
			...
			for i in {1..5}; do
				for _ in {1..10}; do
					$javaCommand -algorithm $algorithm \
						...
						-kmeans.k $k \
						-kmeans.maxiter 100 \
						-kmeans.algorithm HamerlyKMeans \
						-kmeansod.scoring $scoring \
						-algorithm.distancefunction minkowski.EuclideanDistance > $algorithm-$dataset\_$i.out
					awk '/kmeans.k|kmeansod.scoring|runtime: |measures AUROC|Adjusted AUROC|measures AUPRC|Adjusted AUPRC|measures AUPRGC|Adjusted AUPRGC|measures Average Precision|Adjusted AveP|measures R-Precision|Adjusted R-Prec|measures Maximum F1|Adjusted Max F1|measures DCG|Adjusted DCG|measures NDCG /{
						if (/kmeans.k/) {
							kmeans_k = $NF;  # assuming the value is in the last field
						} else if (/kmeansod.scoring/) {
							kmeansod_scoring = $NF;
						...
						print kmeans_k","kmeansod_scoring","auroc","auprc","auprgc","avg_prec","rprec","max_f1","dcg","ndcg","adj_auroc","adj_auprc","adj_auprgc","adj_avg_prec","adj_rprec","adj_max_f1","adj_dcg","runtime;
...
case "$1" in
    ...
    -s) _semantic ;;
	-syn) _synthetic ;;
    *)  echo "Usage: ./kmeansod.sh [-l | -s | -syn]" ;;
esac