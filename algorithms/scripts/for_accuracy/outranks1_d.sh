...
#$((1 + RANDOM % 2147483647))
javaCommand="java -Xmx30G -jar algorithms/binaries/elki-outranks1_d.jar KDDCLIApplication"
algorithm="outlier.subspace.OutRankS1"
alias="outlier.subspace.OutRankS1_D"
...
process_synhtetic_dataset() {
	...
	echo "epsilon,mu,alpha,auroc,auprc,auprgc,avgprec,rprec,f1,dcg,ndcg,adj-auroc,adj-auprc,adj-auprgc,adj-avgprec,adj-rprec,adj-f1,adj-dcg,runtime" >> "$csvFile"
		...
		for epsilon in 0.001 0.01; do
			for i in {1..5}; do
				$javaCommand -algorithm $algorithm \
					...
					-dish.mu $mu \
					-dish.epsilon $epsilon > $algorithm-$dataset\_$i.out
				awk '/dish.mu|dish.epsilon|outrank.s1.alpha|runtime: |measures AUROC|Adjusted AUROC|measures AUPRC|Adjusted AUPRC|measures AUPRGC|Adjusted AUPRGC|measures Average Precision|Adjusted AveP|measures R-Precision|Adjusted R-Prec|measures Maximum F1|Adjusted Max F1|measures DCG|Adjusted DCG|measures NDCG /{
					...
					print dish_mu","dish_epsilon","outrank_s1_alpha","auroc","auprc","auprgc","avg_prec","rprec","max_f1","dcg","ndcg","adj_auroc","adj_auprc","adj_auprgc","adj_avg_prec","adj_rprec","adj_max_f1","adj_dcg","runtime;
...
case "$1" in
    ...
    -s) _semantic ;;
	-syn) _synthetic ;;
    *)  echo "Usage: ./outranks1_d.sh [-l | -s | -syn]" ;;
esac